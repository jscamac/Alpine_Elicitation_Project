...
animal_spp_direction_frequencies <- function(data, Q_IDs) {
  ...
  data %>%
    ...
    select(Expert_ID, Species, SPP_ID, Water_centric, Taxon, Q_TYPE, Q50th) %>%
    ...
    dplyr::group_by(Species, SPP_ID, Taxon, Water_centric) %>%
    dplyr::summarise(N = n(),
    ...
    tidyr::gather(Direction, Responses, -Species, -SPP_ID, -Taxon, -Water_centric, -N, -negative_rank) %>%
