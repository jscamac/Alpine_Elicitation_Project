...
summarise_animal_data <- function(data, Q_IDs, mass_convert = FALSE) {
  ...
  dat <- data %>%
    ...
    dplyr::select(Q_TYPE, Expert_ID, SPP_ID, Plot_ID, Species, Species_short, Water_centric,Taxon, Mass_g, Q50th)
  ...
  dat %>%
    ...
    dplyr::select(Species, Species_short, SPP_ID, Plot_ID, Water_centric, Taxon, Mass_g, N, mean_current, l95ci_current, u95ci_current,
