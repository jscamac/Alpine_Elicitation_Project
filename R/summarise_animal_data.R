...
summarise_animal_data <- function(data, Q_IDs, mass_convert = FALSE) {
  ...
  dat <- data %>%
    ...
    dplyr::select(Q_TYPE, Expert_ID, SPP_ID, Species, Water_centric,Taxon, Mass_g, Q50th)
  ...
  dat %>%
    ...
    dplyr::summarise(N = n(),
                     ...
                     lu95ci_curr = lmn_curr + (1.96 * sd(log(Current))/sqrt(N)),
                     lmn_fut = mean(log(Future)),
                     ll95ci_fut = lmn_fut - (1.96 * sd(log(Future))/sqrt(N)),
                     lu95ci_fut = lmn_fut + (1.96 * sd(log(Future))/sqrt(N)),
